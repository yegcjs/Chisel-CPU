...
class ALU_api_t: public sim_api_t<VerilatorDataWrapper*> {
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_B_in)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_A_in)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_Overflow_out)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_Less)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_Zero)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_ALU_out)));
...
int main(int argc, char **argv, char **env) {
    ...
    VALU* top = new VALU;
    std::string vcdfile = "./generated/ALU/ALU.ALUGen1104683267/ALU.vcd";
