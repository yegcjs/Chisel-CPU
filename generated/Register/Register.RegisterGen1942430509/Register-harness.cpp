...
#endif
#include <iostream>
class Register_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    Register_api_t(VRegister* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_Rd_byte_w_en)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_Rd_in)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_Rd_addr)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_Rt_addr)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_Rs_addr)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_Rt_out)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_Rs_out)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["Register.reset"] = 0;
    }
#if VM_TRACE
...
static Register_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    std::string vcdfile = "./generated/Register/Register.RegisterGen1942430509/Register.vcd";
