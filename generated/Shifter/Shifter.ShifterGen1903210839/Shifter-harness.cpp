...
#endif
#include <iostream>
class Shifter_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    Shifter_api_t(VShifter* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_operation)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_amount)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_in)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_out)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["Shifter.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VShifter* top = new VShifter;
    std::string vcdfile = "./generated/Shifter/Shifter.ShifterGen1903210839/Shifter.vcd";
